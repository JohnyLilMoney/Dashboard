window.initAnimation = function(shadowRoot) {
    ...
    class Fish {
        ...
        setTarget(width, height) {
            ...
            const minX = margin < width - margin ? margin : width * 0.1;
            const maxX = margin < width - margin ? width - margin : width * 0.9;
            const minY = margin < height - margin ? margin : height * 0.1;
            const maxY = margin < height - margin ? height - margin : height * 0.9;
        ...
        draw(ctx) {
            ...
            ctx.beginPath();
            ctx.arc(
                eyeX + this.eyeSize * 0.4,
                -s * 0.1 + this.eyeSize * 0.2,
                this.eyeSize * 0.4,
                0,
                Math.PI * 2
            );
    ...
    let canvas = shadowRoot.getElementById('aquarium-canvas');
    if (!canvas) {
